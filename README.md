...
![csv_contemporary_category](readme_jpg/csv_contemporary.jpg)

et les diffentes couvertures récupérées.

![folder_contemporary_covers](readme_jpg/folder_contemporary_covers.jpg)
...
![folder_output_all](readme_jpg/folder_output_all.jpg)

et le dossier contemporary.

![folder_contemporary](readme_jpg/folder_contemporary.jpg)

\
\
Le script affiche sa progression en cours d'éxécution.

![message_progression](readme_jpg/message_progression.jpg)
...
![option_scrap.py](readme_jpg/option_scrap.py.jpg)
...
Tous les .csv écrits et les couvertures des livres téléchargées via cette option, sont stockés dans le dossier './output/zingle'
    ...
    ![two_books_of_contemporary_category_in_folder_zingle](readme_jpg/two_books_of_contemporary_category_in_folder_zingle.jpg)
...
4.  L'option input, pour recueillir les données d'une ou plusieurs catégories, via un menu à utiliser dans le terminal.
    ...
    ![option_input_menu_home](readme_jpg/option_input_menu_home.jpg)
...
![working_directory](readme_jpg/working_directory.jpg)\
...
![command_line_create_env](readme_jpg/command_line_create_env.jpg)\
...
![env_activate](readme_jpg/env_activate.jpg)


- Sinon, pour activer l'env, il faut lancer la commande :\
![command_line_activate_env](readme_jpg/command_line_activate_env.jpg)
...
![command_line_install_requierements](readme_jpg/command_line_install_requierements.jpg)
...
\
    ...
    -   Unput : L'ensemble des url des livres d'une catégorie (ou l'url du livre renseignée avec l'option 'book').
...
-    _01_sleep_requests_re_attempt.py\
\
    Ce module ajoute volontairement un délai de ,05s avant chaque requests, pour ne pas submerger le site visité et ne pas risquer de s'en faire bannir. En cas d'un code != 200, le module réitére la requests avec un délai un peu plus grand entre deux tentatives. Maximun : 10 tentatives avant abandon.
\
    ![module_01_import_comme_method](readme_jpg/module_01_import_comme_method.jpg)



-   _02_defensive_n_responsive_download.py\
\
    Ce module télécharge les images en ajoutant volontairement un délai de 0,5 avant une tentative, pour les mêmes raisons que celles données pour le module ci-dessus. Le délai entre deux tentatives augmente en fonction du nombre d'essa inaboutit. Maximun : 5 tentatives avant abandon.
    -   Input : L'url d'une image. Le chemin qu'il doit utiliser.
    -   Output : Davantage un Load. L'image téléchargé.
