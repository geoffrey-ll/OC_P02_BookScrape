...
def selection_category_for_scrap(all_url_home_page_category_book):
    ...
    def demand_of_user_what_category_to_scrap():
        all_input_user = [input(('{0}'*56 + '\n#{1:^54}#\n'
                                 ...
                                 '{6:<16} : {7}\n' +
                                 '{8:<16} : {9}\n').format('#',
                                                    'Quelles catégories de livres sont à scraper ?',
                                                    '-noms categories', 'toutes les entrées sur la même ligne',
                                                    '-all', 'TOUTES les catégories',
                                                    '-list', 'catégories disponible au scrap',
    ...
    def analyze_and_collect_input_user(all_input_user):
        for input_user in all_input_user:
            if input_user == ('-list'):
                ...
                count_of_confirm_err = 0
                while count_of_confirm_err == 0:
                    print('\nVous avez choisit TOUTES les catégories :')
                    confirmation_select_category = input(('{0:<5} : {1}\n' +
                                                         '{2:<5} : {3}\n')
                    ...
                    if confirmation_select_category == 'y':
                        for index_cat, cat in enumerate(category_possible):
                            category_validate_to_scrap.append(category_possible[index_cat])
                            count_of_confirm_err += 1
                        ...
                        sys.exit()
                    elif confirmation_select_category != 'y' and confirmation_select_category != 'n':
                        print('\nRéponse invalide')
